...
export default class HomeScreen extends React.Component {
    ...
    register(username, email, password, password2) {
        if (password !== password2) {
            Alert.alert("Paswords don't match.",
                'Please check if both password fields are equal.',
                [{
                    text: 'OK', onPress: () => {
                    }
                }],
        ...
        return (
            <View style={styles.container}>
                ...
                <Image style={styles.logo}
                       source={require('../res/create_account.png')}/>

                <TextInput placeholder="John Appelseed" style={styles.input} onChangeText={(username) => this.setState({ username })} />
                ...
                <TextInput placeholder="Password" style={styles.input} onChangeText={(password) => this.setState({ password })} secureTextEntry={true} />
                <TextInput placeholder="Re-enter password" style={styles.inputLast} onChangeText={(password2) => this.setState({ password2 })} secureTextEntry={true} />

                    <Button
                        style={styles.button}
                        title="Register"
                        onPress={() => this.register( this.state.username, this.state.email, this.state.password, this.state.password2)}
                    />
